...
> **2026-09-22 22:30 更新：这一版已被采用为工程主版本（v3.0）。**
> 工程根目录的 `index.html` 就是它的副本（仅把副标题改成带 `v3.0` 字样）。
> 下面这段保留作参考，说的是「如果它只是存档」时该怎么用。
...
- **已采用为 v3.0 主版本**（提交 `d8b2c3e`，2026-09-22 22:30）；此后主版本在这份档案的基础上继续迭代。
- 只做了内联脚本的语法检查（`node --check` 通过）+ 无头浏览器冒烟测试（0 报错、各交互正常）。
